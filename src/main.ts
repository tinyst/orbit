...
import type { Orbit, OrbitScope, OrbitScopeBehaviorLoader } from "./types.js";
...
export function getOrbit(): Orbit {
  ...
  const scopeLoaders = new Map<string, OrbitScopeBehaviorLoader>();
  const scopes = new WeakMap<Element, OrbitScope>();

  let mutationObserver: MutationObserver | undefined;
  let intersectionObserver: IntersectionObserver | undefined;

  // for first time
  const onInit = (element: Element, scope?: OrbitScope) => {
  ...
  const onAttributeChange = (element: Element, attributeName: string) => {
