...
export function getOrbit(): Orbit {
  ...
  const scopeComponentLoaders = new Map<string, OrbitComponentLoader>();
  ...
  const getStateHooks = (scopeCache: OrbitScopeCache, path: string) => {
  ...
  const registerStateChange = (scopeCache: OrbitScopeCache, element: Element, path: string, hook: OrbitStateHook) => {
  ...
  const notifyStateChange = (scopeCache: OrbitScopeCache, path: string) => {
  ...
  const initDirectives = (element: Element, scopeCache: OrbitScopeCache) => {
  ...
  const initScopeComponent = (element: Element, scopeCache: OrbitScopeCache) => {
  ...
  const initScope = (element: Element) => {
    dbg?.(`init scope`, element);

    const scopeName = element.getAttribute("o-scope") as string;
    ...
    const scopeId = crypto.randomUUID() as OrbitScopeId;
    ...
    getComponent(scopeComponentLoader).then((component) => {
      ...
      const scopeCache: OrbitScopeCache = {
        id: scopeId,
        instance: {
          root: element,
          ...
          state: (initialState, hooks) => {
            ...
            const proxied = proxify(initialState as Record<OrbitScopeCacheKey, any>, {
              path: "",
            });
          ...
          compute: (expression) => {
            ...
            if (!fn) {
              ...
              fn = new Function(`return \`${expression}\`;`) as () => string;
  ...
  const initAdditionalScope = (element: Element) => {
  ...
  // for start
  const initTree = (element: Element) => {
...
export function defineComponent<T extends OrbitComponentProps<any> = undefined>(mount: OrbitComponentFn<T>): OrbitComponent<T> {
  return {
...
// private functions
async function getComponent(loader: OrbitComponentLoader) {
  // if (DEBUG_MODE) {
